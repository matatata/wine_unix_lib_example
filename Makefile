...
mylib.dll: mylib_main.c unixlib.h mylib.spec
	winegcc  -b x86_64-w64-mingw32 -I$(WINE_INCLUDE) -Wl,--wine-builtin -shared mylib_main.c mylib.spec -o mylib.dll 
	
client.exe: client.c mylib.h
	winegcc  -L. -lmylib -b x86_64-w64-mingw32 client.c -o client1.exe
	winegcc -DLOAD_AT_RUNTIME  -b x86_64-w64-mingw32 client.c -o client2.exe


install: mylib.so mylib.dll	client.exe
# copy exe
	 cp client1.exe client2.exe "$(WINEPREFIX)/drive_c/"

# copy builtins .. that might be enough ...
	 sudo cp mylib.dll "$(WINE_LIB_DIR)/x86_64-windows/"
	 sudo cp mylib.so "$(WINE_LIB_DIR)/x86_64-unix/"
	 
# ... but placing the dll next to the exe is enough
# cp mylib.dll "$(WINEPREFIX)/drive_c/"

# or copying to system32 which frees us from keeping it around:
# cp mylib.dll "$(WINEPREFIX)/drive_c/windows/system32/"
...
run_no_install: clean mylib.so mylib.dll client.exe
	...
	WINEPATH=$(PWD) WINEDLLPATH=$(PWD) WINEDEBUG=-all MVK_CONFIG_LOG_LEVEL=0 wine client1.exe
	WINEPATH=$(PWD) WINEDLLPATH=$(PWD) WINEDEBUG=-all MVK_CONFIG_LOG_LEVEL=0 wine client2.exe
...
clean:
	...
	rm -f "$(WINEPREFIX)/drive_c/client1.exe"
	rm -f "$(WINEPREFIX)/drive_c/client2.exe"
